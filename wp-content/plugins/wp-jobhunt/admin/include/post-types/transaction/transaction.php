...
if (!function_exists('transactions_columns_add')) {
    ...
    function transactions_columns_add($columns) {
        unset($columns['title']);
        unset($columns['date']);
        $columns['p_title'] = __('Package Id', 'automobile');
        $columns['p_date'] = __('Date', 'automobile');
        $columns['users'] = __('User', 'automobile');
        $columns['package'] = __('Package Name', 'automobile');
        $columns['gateway'] = __('Payment Gateway', 'automobile');
        $columns['amount'] = __('Amount', 'automobile');
        return $columns;
    }
        ...
        switch ($name) {
            ...
            case 'gateway':
                ...
                if ($cs_trans_gate != '') {
                    ...
                    echo CS_FUNCTIONS()->cs_special_chars($cs_trans_gate);
                } else {
                    echo '-';
                }
                break;
...
if (!class_exists('post_type_transactions')) {
    ...
    class post_type_transactions {
        ...
        public function transactions_register() {
            $labels = array(
                'name' => __('Packages', 'automobile'),
                'menu_name' => __('Packages', 'automobile'),
                'add_new_item' => __('Add New Packages', 'automobile'),
                'edit_item' => __('Edit Packages', 'automobile'),
                'new_item' => __('New Packages Item', 'automobile'),
                'add_new' => __('Add New Packages', 'automobile'),
                'view_item' => __('View Packages Item', 'automobile'),
                'search_items' => __('Search', 'automobile'),
                'not_found' => __('Nothing found', 'automobile'),
                'not_found_in_trash' => __('Nothing found in Trash', 'automobile'),
                'parent_item_colon' => ''
            );
            $args = array(
                'labels' => $labels,
        ...
        public function cs_meta_transactions_add() {
            add_meta_box('cs_meta_transactions', __('Packages Options', 'automobile'), array(&$this, 'cs_meta_transactions'), 'cs-transactions', 'normal', 'high');
        }
        ...
        public function cs_meta_transactions($post) {
            ...
            $transaction_meta['transaction_id'] = array(
                'name' => 'transaction_id',
                'type' => 'hidden_label',
                'title' => __('Transaction Id', 'automobile'),
                'description' => '',
            );
            $transaction_meta['transaction_user'] = array(
                ...
                'title' => __('Package User', 'automobile'),
            ...
            $transaction_meta['transaction_pay_method'] = array(
                ...
                'type' => 'select',
                'classes' => 'chosen-select-no-single',
                'title' => __('Payment Gateway', 'automobile'),
            ...
            $transaction_meta['transaction_expiry_date'] = array(
                ...
                'type' => 'text',
                'title' => __('Package Expiry Date', 'automobile'),
                'description' => '',
            );
            if ($cs_trans_type == 'cv_trans') {
                $transaction_meta['transaction_listings'] = array(
                    ...
                    'type' => 'text',
                    'title' => __('No. of CV\'s', 'automobile'),
                    'description' => '',
                );
            } else {
                $transaction_meta['transaction_listings'] = array(
                    'name' => 'transaction_listings',
                    'type' => 'text',
                    'title' => __('No. of Listings', 'automobile'),
                    'description' => '',
                );
            }
            ...
            if ($cs_trans_type != 'cv_trans') {
                $transaction_meta['transaction_listing_expiry'] = array(
                    ...
                    'type' => 'text',
                    'title' => __('Listing Expiry', 'automobile'),
                    'description' => '',
                );
                $transaction_meta['transaction_listing_period'] = array(
                    ...
                    'type' => 'select',
                    'classes' => 'chosen-select-no-single',
                    'title' => __('Listing Period', 'automobile'),
                    'options' => array('days' => __('Days', 'automobile'), 'months' => __('Months', 'automobile'), 'years' => __('Years', 'automobile')),
                    'description' => '',
                );
                $transaction_meta['transaction_ex_features'] = array(
                    'name' => 'transaction_ex_features',
                    'type' => 'extra_features',
                    'title' => __('Jobs', 'automobile'),
                    'description' => '',
                );
            }
            if ($cs_trans_type == 'cv_trans') {
