    ...
    class cs_employer_functions {
        ...
        public function cs_create_user($cs_username, $cs_user_email) {
            ...
            if (!username_exists($cs_username) && !email_exists($cs_user_email)) {
                ...
                if (!is_wp_error($cs_register)) {
                    ...
                    // Creating Employer Post
                    $employer_post = array(
                        'post_title' => $cs_username,
                        'post_content' => '',
                        'post_status' => 'publish',
                        'post_author' => '',
                        'post_type' => 'employer',
                    ...
                    $post_id = wp_insert_post($employer_post);
                    if ($post_id) {
                        // update username
                        update_post_meta($post_id, 'cs_user', $get_user->ID);
                    }
                    return $get_user->ID;
                }
            }
                ...
                foreach ($cs_job_cus_fields as $cus_field) {
                    ...
                    switch ($cs_type) {
                        case('text'):
                            ...
                            $cs_html .= '
                                        <div class="col-md-6">
                                            <label>asd' . esc_attr($cs_label) . '</label>';
                            $cs_opt_array = array(
        ...
        public function cs_add_job($values = array(), $cs_elem = false) {
            ...
            if ($cs_elem == true) {
                ...
                $cs_job_desc = html_entity_decode(base64_decode($cs_job_desc));
            }
            $job_post = array(
        ...
        public function cs_pay_process($values = array()) {
            ...
            if (isset($_POST['cs_payment_gateway']) && $_POST['cs_payment_gateway'] == 'cs_pre_bank_transfer') {
                $cs_trans_pay_method = 'cs_pre_bank_transfer';
            }
            ...
            if ($cs_trans_amount <= 0) {
                $cs_trans_array['transaction_status'] = 'approved';
            }
            ...
            if ($cs_trans_amount > 0) {
                ...
                if (isset($_POST['cs_payment_gateway']) && $_POST['cs_payment_gateway'] == 'CS_PAYPAL_GATEWAY' && !empty($cs_transaction_fields)) {
...
// Start function for cv package sub
        ...
        public function cs_fav_resume_del() {
            ...
            $cs_emp_id = $cs_emp_funs->cs_get_post_id_by_meta_key("cs_user", $cs_emp_id);
        ...
        public function cs_check_emp_resume($id) {
            global $current_user;
            $cs_emp_id = $this->cs_get_post_id_by_meta_key('cs_user', $current_user->ID);
            $cs_fav_resumes = get_post_meta($cs_emp_id, "cs_fav_resumes", true);
        ...
        public function cs_header_favorites() {
            ...
            } else if (is_user_logged_in() && !$this->is_employer()) {
                echo '<div class="wish-list"> 
                        <a><i class="icon-heart6"></i></a> <em class="cs-bgcolor" id="cs-fav-counts">0</em>
                       </div>';
            } else if (is_user_logged_in() && $this->is_employer()) {
                        ...
                        <div class="recruiter-widget wish-list-dropdown">
                            <ul class="recruiter-list">
                                ...
                                foreach ($exist_vals as $ex_val) {
                                    ...
                                    $cs_user_img = get_post_meta($ex_val, "user_img", true);
                                    $cs_job_title = get_post_meta($ex_val, "cs_job_title", true);
                                    $cs_loc_address = get_post_meta($ex_val, "cs_post_loc_address", true);
                                    ?>
                                    <li class="alert alert-dismissible">
                                        <button class="close" id="cs-rem-<?php echo absint($ex_val) ?>" onclick="cs_unset_user_fav('<?php echo esc_js(admin_url('admin-ajax.php')); ?>', '<?php echo esc_js($ex_val) ?>')"><span>&times;</span></button>
                                        <?php
                                        if ($cs_user_img != '') {
                                            $cs_user_img = cs_get_img_url($cs_user_img, 'cs_media_5');
                                            ?>
                                            <a href="<?php echo esc_url(get_the_permalink($ex_val)) ?>"><img src="<?php echo esc_url($cs_user_img) ?>" alt="" /></a>
                                        <?php } ?>
                                        <div class="cs-info">
                                            <h4><a href="<?php echo esc_url(get_the_permalink($ex_val)) ?>"><?php echo get_the_title($ex_val) ?></a></h4>
                                            <?php if ($cs_loc_address != '') { ?>
                                                <span class="location"><i class="icon-location6"></i><?php echo CS_FUNCTIONS()->cs_special_chars($cs_loc_address) ?></span>
                                                <?php } ?>
                                        </div>
                                    </li>
                                    <?php
            ...
            if (isset($_POST['cs_user']) && $_POST['cs_user'] <> '') {
                ...
                if (isset($_POST['user_email'])) {
                    ...
                    if (isset($email_response->errors)) {
                        echo __('Your given email address is already used or invalid, Please try with correct information', 'jobdoor');
                        die();
                    }
                }
                // update display name
                ...
                echo __('Update Successfully', 'jobdoor');
            } else {
                echo "Save Failed";
            }
            die();
