...
if (!function_exists('jobcareer_pb_register')) {
    ...
    function jobcareer_pb_register($die = 0) {
        ...
        <div id="<?php echo esc_attr($name . $cs_counter); ?>_del" class="column  parentdelete <?php echo esc_attr($coloumn_class); ?> <?php echo esc_attr($shortcode_view); ?>" item="register" data="<?php echo cs_element_size_data_array_index($button_element_size) ?>" >
            ...
            <div class="cs-wrapp-class-<?php echo esc_attr($cs_counter) ?> <?php echo esc_attr($shortcode_element); ?>" id="<?php echo esc_attr($name . $cs_counter) ?>" data-shortcode-template="[cs_register {{attributes}}]" style="display: none;">
                <div class="cs-heading-area">
                    <h5><?php _e('Edit Register Options', 'jobs'); ?></h5>
                ...
                <div class="cs-pbwp-content">
                    ...
                    <?php if (isset($_POST['shortcode_element']) && $_POST['shortcode_element'] == 'shortcode') { ?>
                        <ul class="form-elements insert-bg">
                            <li class="to-field"> <a class="insert-btn cs-main-btn" onclick="javascript:Shortcode_tab_insert_editor('<?php echo esc_js(str_replace('jobcareer_pb_', '', $name)); ?>', '<?php echo esc_js($name . $cs_counter) ?>', '<?php echo esc_js($filter_element); ?>')" ><?php _e('Insert', 'jobs'); ?></a> </li>
                        </ul>
    ...
    function cs_recover_pass() {
        ...
        if (isset($_POST['action']) && 'cs_recover_pass' == $_POST['action']) {
            ...
            if (empty($email)) {
                ...
                if ($update_user) {
                    $to = $email;
                    $subject = __('Your new password', 'jobhunt');
                    $sender = get_option('name');
                    $message = __('Your request has been completed succssfully, Now you can use following information for login.', 'jobhunt') . "\r\n";
                    $message .= __('Your username is: ', 'jobhunt') . $username;
                    $message .= __('Your new password is: ', 'jobhunt') . $random_password;
    ...
    function cs_lost_pass($atts, $content = "") {
        ...
        if ($cs_type == 'popup') {
            ...
            <div class="modal-header">
                <h2><?php _e('Forgot Password', 'jobhunt') ?></h2>
