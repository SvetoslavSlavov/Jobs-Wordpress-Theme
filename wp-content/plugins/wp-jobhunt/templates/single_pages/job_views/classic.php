                            ...
                            <aside class="col-md-4 employer-info">
                                ...
                                if (isset($employer_post) && $employer_post != '') {
                                    ...
                                    <div class="employer-company">
                                        <figure><a href="<?php echo esc_url(get_permalink($employer_post->ID)); ?>"><img src="<?php echo esc_url($cs_employee_employer_img); ?>" alt="<?php echo esc_html($employer_post->post_title); ?>" /></a><span class="cs-bgcolor"><?php echo esc_html($count_employer_jobs); ?></span></figure>
                                        <div class="cs-text">
                                            <h4><a href="<?php echo esc_url(get_permalink($employer_post->ID)); ?>"><?php echo esc_html($employer_post->post_title); ?></a></h4>
                                            <ul class="detail-list">
                                                <?php if ($cs_employee_address != '') echo '<li><address> ' . $cs_employee_address . '</address></li>'; ?>
                                                <?php
                                                if ($cs_employee_web != '') {
                                                        ...
                                                        if (isset($user_role) && $user_role <> '' && $user_role == 'cs_candidate') {
                                                            ...
                                                            if (isset($user) and $user <> '' and is_user_logged_in()) {
                                                                ...
                                                                if (is_array($finded_result_list) && !empty($finded_result_list)) {
                                                                    ?>
                                                                    <a class="like applied_icon" onclick="cs_removejobs_left_to_applied('<?php echo esc_url(admin_url('admin-ajax.php')); ?>', '<?php echo intval($job_post->ID); ?>', this)" >
                                                                        <span><i class="icon-thumbsup"></i></span><?php _e('Applied', 'jobhunt') ?>
                                                                    </a>
                                                                    <?php
                                                                } else {
                                                                    ?>
                                                                    <a class="like applied_icon" onclick="cs_addjobs_left_to_applied('<?php echo esc_url(admin_url('admin-ajax.php')); ?>', '<?php echo intval($job_post->ID); ?>', this)" >
