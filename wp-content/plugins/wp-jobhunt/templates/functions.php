...
if (!function_exists('cs_show_all_cats')) {
    ...
    function cs_show_all_cats($parent, $separator, $selected = "", $taxonomy, $optional = '') {
        ...
        if ($optional) {
            $a_options = array();
            $a_options[''] = __("Please select..", 'jobs');
        ...
        if ($admin_email != '' && filter_var($admin_email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($error == 0) {
                ...
                if ($respose) {
                    $error = 0;
                    $error_result = 0;
                    $results = __("&nbsp; <span style=\"color: #060;\">Your inquery has been sent User will contact you shortly</span>", "jobhunt");
                } else {
                    $error = 1;
                    $error_result = 1;
...
if (!function_exists('ajaxcontact_employer_send_mail')) {
    ...
    function ajaxcontact_employer_send_mail() {
        ...
        if ($admin_email != '' && filter_var($admin_email, FILTER_VALIDATE_EMAIL)) {
            ...
            $contents = 'Name:' . $name . "\n" . 'Email:' . $email . "\n" . 'Phone:' . $phone . "\n" . $contents;
            if ($error == 0) {
                $respose = wp_mail($admin_email, $subject, $contents, $headers, '');

                if ($respose) {
                    $error_result = 0;
                    $results = "&nbsp; <span style=\"color: #060;\">".__('Your inquery has been sent User will contact you shortly', 'jobhunt')."</span>";
                } else {
                    $error_result = 1;

                    $results = "&nbsp; <span style=\"color: #ff0000;\">*".__('The mail could not be sent due to some resons, Please try again', 'jobhunt')."</span> ";
                }
            }
        } else {
            $results = "&nbsp; <span style=\"color: #ff0000;\">**".__('The profile email does not exist, Please try later.', 'jobhunt')."</span> ";
...
if (!function_exists('cs_get_custom_locationswith_google_auto')) {
    ...
    function cs_get_custom_locationswith_google_auto($dropdown_start_html = '', $dropdown_end_html = '', $cs_text_ret = false, $cs_top_search = false) {
        ...
        /* if ($cs_top_search == true) {
          $cs_opt_array['cust_id'] = 'cs_search_top_location_field';
          } else {
          $cs_opt_array['cust_id'] = 'cs_search_location_field';
          } */
        ...
        if (isset($user) && $user <> '') {
            if (isset($_POST['post_id']) && $_POST['post_id'] <> '') {
                ...
                <div class="outerwrapp-layer<?php echo esc_html($_POST['post_id']); ?> cs-added-msg">
                    <?php echo __('Added to Selected List', 'jobhunt'); ?>
                </div>
                <?php
            }
        } else {
...
if (!function_exists('get_job_type_dropdown')) {
    ...
    function get_job_type_dropdown($name, $id, $selected_post_id = '', $class = '', $required_status = 'false') {
        ...
        $select_options[''] = __('Please Select', 'jobhunt');
...
if (!function_exists('get_job_specialisms_dropdown')) {
    ...
    function get_job_specialisms_dropdown($name, $id, $selected_post_id = '', $class = '', $required_status = 'false') {
        ...
        $select_options[''] = __('Please Select specialism', 'jobhunt');
...
if (!function_exists('cs_get_img_url')) {
    ...
    function cs_get_img_url($img_name = '', $size = 'cs_media_2', $return_sizes = false) {
        $ret_name = '';
        $cs_img_sizes = array(
            'cs_media_1' => '-870x489',
            'cs_media_2' => '-270x203',
...
//                    }
                ...
                $top_wishlist_menu_html .='<li class="alert alert-dismissible"><a href="' . esc_url(cs_users_profile_link($cs_page_id, 'shortlisted_jobs', $user)) . '" >View All</a></li>
                    </ul>
                </div></div>';
                wp_reset_postdata();
            endif;
...
if (!function_exists('check_candidate_applications')) {
    ...
    function check_candidate_applications($candidate_meta_id) {
        ...
        if (is_user_logged_in() && $cs_emp_funs->is_employer()) {
            ...
            $cs_jobapplied_array = get_user_meta($candidate_id, 'cs-user-jobs-applied-list', true);
            if (!empty($cs_jobapplied_array))
                $cs_jobapplied = array_column_by_two_dimensional($cs_jobapplied_array, 'post_id');
            else
                $cs_jobapplied = array();
...
if (!function_exists('wp_new_user_notification')) :
    ...
    function wp_new_user_notification($user_id, $plaintext_pass = '') {
        ...
        wp_mail($user_email, sprintf(__('[%s] Your username and password ', 'jobhunt'), $blogname), $message);
    }
...
if (!function_exists('cs_job_map')) {
    ...
    function cs_job_map($atts, $content = "") {
        ...
        wp_enqueue_script('cs_google_autocomplete_script', 'http://maps.google.com/maps/api/js?sensor=false&libraries=places', '', '', true);
        $map_type = isset($map_type) ? $map_type : '';
        $map_dynmaic_no = cs_generate_random_string('10');
        $html = '';
        $html .= '<div  class="' . $cs_map_class . ' " style="animation-duration:">';
