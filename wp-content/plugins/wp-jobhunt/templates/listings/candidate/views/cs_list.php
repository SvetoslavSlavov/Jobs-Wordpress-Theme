...
if (is_user_logged_in() && !$login_user_is_employer_flag) {
    ?>
    <div id="cs-not-emp" style="display:none;"><?php _e('Oppsss!! You are not logged in as employer to shortlist applicant.', 'jobhunt') ?></div>
    <?php
}
?>
<!--start demo demo================================================================-->
    ...
    if ($count_post > 0) {
        ...
        if (!empty($loop->results)) {
            foreach ($loop->results as $cs_user) {
                $cs_post_loc_city = get_user_meta($cs_user->ID, 'cs_post_loc_city', true);
                $cs_candidate_web_http = get_user_meta($cs_user->ID, 'cs_website', true);
                    ...
                    <div class="cs-media">
                        <figure>
                            <a <?php echo CS_FUNCTIONS()->cs_special_chars($cs_href) ?>>
                                <img src="<?php echo esc_url($candidate_img) ?>" alt="" />
                            </a>
                        </figure>
                    </div>
                    <div class="cs-text">
                        <div class="cs-post-title">
                            <h5><a <?php echo CS_FUNCTIONS()->cs_special_chars($cs_href) ?> ><?php echo $cs_user->display_name ?></a> <?php if ($cs_post_loc_city != '') { ?><span class="cs-location cs-color"><?php echo CS_FUNCTIONS()->cs_special_chars($cs_post_loc_city) ?></span><?php } ?></h5>
                        </div>
                        <div class="post-option">
                            ...
                            if ($cs_minimum_salary != '') {
                                echo '<span class="cs-min-salary">' . $default_currency_sign . $cs_minimum_salary . __(' p.a minimum', 'jobhunt') . '</span>';
                            }
                            if ($cs_user_last_activity_date != '') {
                                ?>
                                <span class="cs-post-date"><?php echo __('Last Activity') . ' ' . cs_time_elapsed_string($cs_user_last_activity_date); ?></span>
                            <?php } ?>
                        </div>
                        <?php echo force_balance_tags($specialism_html) ?>
                    </div>
                        ...
                        <span>
                            ...
                            if ($cs_candidate_switch == 'on') {
                                if (!is_user_logged_in()) {
                                    ?>
                                    <a data-toggle="tooltip" data-placement="top" title="<?php echo __("Add to List", "jobhunt"); ?>" class="add_list_icon cs-button" onclick="trigger_func('#btn-header-main-login');"><?php _e('Add to List', 'jobhunt') ?></a>
                                    <?php
                                } else if (is_user_logged_in() && !$login_user_is_employer_flag) {
                                    ?>
                                    <a data-toggle="tooltip" data-placement="top" title="<?php echo __("Add to List", "jobhunt"); ?>" class="add_list_icon cs-button" id="cs_empl_check_<?php echo absint($cs_candidate_user) ?>" data-id="<?php echo absint($cs_candidate_user) ?>"><?php _e('Add to List', 'jobhunt') ?></a>
                                    <?php
                                } else if (is_user_logged_in() && $login_user_is_employer_flag) {
                                    if (is_array($cs_fav_resumes) && in_array($cs_candidate, $cs_fav_resumes)) {
                                        ?>
                                        <a data-toggle="tooltip" data-placement="top" title="<?php echo __("Add to List", "jobhunt"); ?>" <?php echo CS_FUNCTIONS()->cs_special_chars($cs_href) ?> class="add_list_icon cs_resume_added cs-button" id="add-to-btn-<?php echo absint($cs_candidate_user) ?>"><?php _e('View Detail', 'jobhunt') ?></a>
                                        <?php
                                    } else {
                                        ?>
                                        <a data-toggle="tooltip" data-placement="top" title="<?php echo __("Add to List", "jobhunt"); ?>" class="add_list_icon cs-button" id="add-to-btn-<?php echo absint($cs_candidate_user) ?>" onclick="cs_add_to_list('<?php echo esc_js(admin_url('admin-ajax.php')); ?>', '<?php echo absint($cs_candidate_user) ?>')"><?php _e('Add to List', 'jobhunt') ?></a>
                                        <?php
                                    }
                                }
                            } else {
                                ...
                                if (!is_user_logged_in()) {
                                    ?>
                                    <a data-toggle="tooltip" data-placement="top" title="<?php echo __("Shortlist", "jobhunt"); ?>" class="add_list_icon cs-button" onclick="trigger_func('#btn-header-main-login');"><i class="icon-plus-circle"></i><?php _e('Shortlist', 'jobhunt') ?></a>
                                    <?php
                                } else if (is_user_logged_in() && !$login_user_is_employer_flag) {
                                    ?>
                                    <a data-toggle="tooltip" data-placement="top" title="<?php echo __("Shortlist", "jobhunt"); ?>" class="add_list_icon cs-button" id="cs_empl_check_<?php echo absint($cs_candidate_user) ?>" data-id="<?php echo absint($cs_candidate_user) ?>"><i class="icon-plus-circle"></i><?php _e('Shortlist', 'jobhunt') ?></a>
                                    <?php
                                } else if (is_user_logged_in() && $login_user_is_employer_flag) {
                                    ...
                                    if (is_array($finded_result_list) && !empty($finded_result_list)) {
                                        ?>
                                        <a data-toggle="tooltip" data-placement="top" title="<?php echo __("Shortlisted", "jobhunt"); ?>" href="javascript:void(0);" class="add_list_icon cs_resume_added cs-button" id="add-to-btn-<?php echo absint($cs_candidate_user) ?>" onclick="cs_add_favr('<?php echo esc_js(admin_url('admin-ajax.php')); ?>', '<?php echo absint($cs_candidate_user) ?>', 'remove')"><i class="icon-minus-circle"></i><?php _e('Shortlisted', 'jobhunt') ?></a>
                                        <?php
                                    } else {
                                        ?>
                                        <a data-toggle="tooltip" data-placement="top" title="<?php echo __("Shortlist", "jobhunt"); ?>" class="add_list_icon cs-button" id="add-to-btn-<?php echo absint($cs_candidate_user) ?>" onclick="cs_add_favr('<?php echo esc_js(admin_url('admin-ajax.php')); ?>', '<?php echo absint($cs_candidate_user) ?>', 'add')"><i class="icon-plus-circle"></i><?php _e('Shortlist', 'jobhunt') ?></a>
                                            <?php
                                        }
                                    }
                                }
