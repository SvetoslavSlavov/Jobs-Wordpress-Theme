...
if (!function_exists('cs_theme')) {
    ...
    function cs_theme() {
        add_theme_page("Import Demo Data", __("Import Demo Data", 'jobhunt'), 'read', 'cs_demo_importer', 'cs_demo_importer');
    }
    ...
    function cs_demo_importer() {
        ...
        if (isset($_REQUEST['demo']) && $_REQUEST['demo'] == 'demo-data') {
            ...
            if (!class_exists('WP_Import')) {
                ...
                $cs_import_class = cs_framework::plugin_dir() . 'include/cs-importer/wordpress-importer.php';
                if (file_exists($cs_import_class))
                    require_once $cs_import_class;
                else
                    $cs_demoimport_error = true;
            ...
            if ($cs_demoimport_error) {
                echo __('Error.', 'jobhunt') . '</p>';
                die();
            } else {

                if (!is_file(cs_framework::plugin_dir() . 'include/cs-importer/demo.xml')) {
                    echo '<p><strong>' . __('Sorry, there has been an error.', 'jobhunt') . '</strong><br />';
                    echo __('The file does not exist, please try again.', 'jobhunt') . '</p>';
                } else {
                    if (!get_option('cs_import_demo')) {
                        ...
                        $cs_demo_import->import(cs_framework::plugin_dir() . 'include/cs-importer/demo.xml');
                        ...
                        // import all users and usermeta
                        if (class_exists('cs_user_import')) {
                            $cs_user_import = new cs_user_import();
                            $results = $cs_user_import->cs_import_user_demodata();
                            echo '<p><strong>' . __('Users Settings Saved.', 'jobhunt') . '</strong><br />';
                        }
                        ...
                        echo '<p><strong>' . __('You have already install this demo.', 'jobhunt') . '</strong><br />';
                    }
                }
            }
        }
        ...
        <div class="cs-demo-data">
            <h2><?php _e('Import Demo Data', 'jobhunt'); ?></h2>

            <div class="inn-text">
                <p><?php _e('Importing demo data helps to build site like the demo site by all means. It is the quickest way to setup theme. Following things happen when dummy data is imported;', 'jobhunt'); ?></p>
                <ul class="import-data">
                    <li>&#8226;<?php _e('All wordpress settings will remain same and intact', 'jobhunt'); ?></li>
                    <li>&#8226;<?php _e('Posts, pages and dummy images shown in demo will be imported', 'jobhunt'); ?></li>
                    <li>&#8226;<?php _e('Only dummy images will be imported as all demo images have copy right restriction', 'jobhunt'); ?> </li>
                    <li>&#8226;<?php _e('No existing posts, pages, categories, custom post types or any other data will be deleted or modified', 'jobhunt'); ?></li>
                    <li>&#8226;<?php _e('To proceed, please click "Import Demo Data" and wait for a while', 'jobhunt'); ?></li>
                    <li style="color:red;">&#8226; <b><?php _e('Note: Before Import demo data please make sure your server has following Setting', 'jobhunt'); ?></b></li>
                    <li>
                        <ul style="margin-left:15px;">
                            <li>&#8226;<?php _e('post_max_size = 10M or Greater', 'jobhunt'); ?> </li>
                            <li>&#8226;<?php _e('upload_max_filesize = 10M or Greater', 'jobhunt'); ?> </li>
                            <li>&#8226;<?php _e('memory_limit = 128M or Greater', 'jobhunt'); ?></li>
                        </ul>
                    </li>
                    <li>&#8226;<?php _e('Your Current Setting', 'jobhunt'); ?> </b></li>
                    <li>
                        <ul style="margin-left:15px;">
                            <?php
                            ...
                            <div class="meta-input pattern">
                                ...
                                foreach ($cs_page_option['theme_options']['select'] as $key => $value) {
								  ...
								  <label for="radio_option_1"> 
									  <span class="ss"><img src="' . esc_url(get_template_directory_uri()) . '/backend/assets/images/import/' . $cs_page_option['theme_options'][$key]['thumb'] . '.png" /></span> 
...
if (!function_exists('cs_update_themeoptions')) {
    ...
    function cs_update_themeoptions($page = '', $formdata = '', $theme_option = '') {
        ...
        if (isset($page->ID)) {
            ...
            echo '<p><strong>' . __('Theme Settings Saved.', 'jobhunt') . '</strong><br />';

            if (function_exists('cs_demo_plugin_data')) {
                cs_demo_plugin_data();
            }

            echo '<p><strong>' . __('Plugin Settings Saved.', 'jobhunt') . '</strong><br />';
        } else {
