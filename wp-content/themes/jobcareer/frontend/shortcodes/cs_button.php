...
if (!function_exists('jobcareer_button_shortcode')) {
    ...
    function jobcareer_button_shortcode($atts) {
        ...
        $html = '';
        $html .= '<div ' . $CustomId . ' class="button_style cs-button">';

        $html .= '<a href="' . esc_url($button_link) . '" class="btn-post ' . sanitize_html_class($button_type_class) . '  ' . $button_size . ' bg-color ' . $cs_button_class . '  ' . $has_icon . '" style="' . $border . '  background-color: ' . $button_bg_color . '; color:' . $button_color . ';" target="' . $button_target . '">';
        if (isset($button_icon) && $button_icon <> '') {
