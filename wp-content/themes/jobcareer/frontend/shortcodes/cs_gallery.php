...
if (!function_exists('jobcareer_gallery_shortcode')) {
    ...
    function jobcareer_gallery_shortcode($atts, $content = "") {
         ...
         $args = array(
            ...
            if ($cs_gal_layout == 'gallery-slider') {
                ...
                if ($cs_meta_gallery_options <> "") {
                    for ($i = $limit_start; $i < $limit_end; $i++) {
                        ...
                        if (isset($image_url_full) && $image_url_full != "") {
                            $html .= '<li data-thumb="' . $image_url_full . '"><img src="' . esc_url($image_url_full) . '" alt=' . $image_title . '></li>';
                        }
                    }
                }
                $html .= '</ul>';
                ...
                if ($cs_meta_gallery_options <> "") {
                    for ($i = $limit_start; $i < $limit_end; $i++) {
                        ...
                        if (isset($image_url_full) && $image_url_full != "") {
                            ...
                            $html .= '<figure>';
                            $html .= '<a data-rel="prettyPhoto[gallery-four-col]>" data-title="' . $image_title . '"  href="' . esc_url($image_url_full) . '">' . "<img src='" . esc_url($image_url_full) . "' data-alt='" . $image_title . "' alt='" . $image_title . "' />" . '</a>';
              ...
              if (isset($image_url_full) && $image_url_full != "") {
                            ...
                            $html .= '<figure>';
                            $html .= '<a data-rel="prettyPhoto[gallery-wordpress]>" data-title="' . $image_title . '"  href="' . esc_url($image_url_full) . '">' . "<img src='" . esc_url($image_url_full) . "' data-alt='" . $image_title . "' alt='" . $image_title . "' />" . '</a>';
        ...
        if ($cs_gal_pagination == "Show Pagination" and $count_post > $cs_gal_media_per_page and $cs_gal_media_per_page > 0) {
                ...
                if ($cs_meta_gallery_options <> "") {
                    ...
                    for ($i = $limit_start; $i < $limit_end; $i++) {
                        ...
                        if (isset($image_url_full) && $image_url_full != "") {
                            $html .= '<article class="item col-md-3 has_border">';
                            $html .= '<figure>';
                            $html .= '<a data-rel="prettyPhoto[gallery-masonry]>" target="" data-title="' . $image_title . '"  href="' . esc_url($image_url_full) . '">' . "<img src='" . esc_url($image_url_full) . "' data-alt='" . $image_title . "' alt='" . $image_title . "' />" . '</a>';
                ...
                if ($cs_gal_pagination == "Show Pagination" and $count_post > $cs_gal_media_per_page and $cs_gal_media_per_page > 0) {
                    ...
                    $html .= cs_pagination($count_post, $cs_gal_media_per_page, $qrystr, esc_html__('Show Pagination','jobcareer'));
                }
    ...
    if (function_exists('cs_short_code')) {
        cs_short_code('cs_gallery', 'jobcareer_gallery_shortcode');
    }
}

// start Albums item function
if (!function_exists('cs_album_item_shortcode')) {
    ...
    function cs_album_item_shortcode($atts, $content = "") {
        ...
        if (isset($cs_album_track_mp3_url) && !empty($cs_album_track_mp3_url)) {
            $html .= '<source src="' . esc_url($cs_album_track_mp3_url) . '" title="' . $cs_album_track_title . '">';
        }
        return $html;
    }

    if (function_exists('cs_short_code')) {
        cs_short_code('album_item', 'cs_album_item_shortcode');
    }
}
