    ...
    class TGM_Plugin_Activation {
        ...
        public function init() {
            ...
            $this->strings = array(
                ...
                'installing' => esc_html__('Installing Plugin: %s', 'jobcareer'),
                'oops' => esc_html__('Something went wrong.', 'jobcareer'),
                'notice_can_install_required' => _n_noop(
                        'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'jobcareer'
                ),
                'notice_can_install_recommended' => _n_noop(
                        'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'jobcareer'
                ),
                'notice_cannot_install' => _n_noop(
                        'Sorry, but you do not have the correct permissions to install the %s plugin.', 'Sorry, but you do not have the correct permissions to install the %s plugins.', 'jobcareer'
                ),
                'notice_can_activate_required' => _n_noop(
                        'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'jobcareer'
                ),
                'notice_can_activate_recommended' => _n_noop(
                        'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'jobcareer'
                ),
                'notice_cannot_activate' => _n_noop(
                        'Sorry, but you do not have the correct permissions to activate the %s plugin.', 'Sorry, but you do not have the correct permissions to activate the %s plugins.', 'jobcareer'
                ),
                'notice_ask_to_update' => _n_noop(
                        'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'jobcareer'
                ),
                'notice_cannot_update' => _n_noop(
                        'Sorry, but you do not have the correct permissions to update the %s plugin.', 'Sorry, but you do not have the correct permissions to update the %s plugins.', 'jobcareer'
                ),
                'install_link' => _n_noop(
                        'Begin installing plugin', 'Begin installing plugins', 'jobcareer'
                ),
                'activate_link' => _n_noop(
                        'Begin activating plugin', 'Begin activating plugins', 'jobcareer'
                ),
                'return' => esc_html__('Return to Required Plugins Installer', 'jobcareer'),
                'dashboard' => esc_html__('Return to the dashboard', 'jobcareer'),
                'plugin_activated' => esc_html__('Plugin activated successfully.', 'jobcareer'),
                'activated_successfully' => esc_html__('The following plugin was activated successfully:', 'jobcareer'),
                'complete' => esc_html__('All plugins installed and activated successfully. %1$s', 'jobcareer'),
                'dismiss' => esc_html__('Dismiss this notice', 'jobcareer'),
                'contact_admin' => esc_html__('Please contact the administrator of this site for help.', 'jobcareer'),
            );
        ...
        public function admin_init() {
            ...
            if (isset($_REQUEST['tab']) && 'plugin-information' === $_REQUEST['tab']) {
                ...
                jobcareer_include_file(ABSPATH . 'wp-admin/includes/plugin-install.php');
        ...
        public function install_plugins_page() {
            ...
            <div class="tgmpa wrap">
                ...
                <form id="tgmpa-plugins" action="" method="post">
                    ...
                    $cs_opt_array = array(
                        'std' => esc_attr($this->menu),
                        'id' => '',
                        'classes' => '',
                        'extra_atr' => '',
                        'cust_id' => '',
                        'cust_name' => 'tgmpa-page',
                        'return' => true,
                        'required' => false
                    );
        ...
        protected function do_plugin_install() {
            ...
            if (isset($this->plugins[$slug], $_GET['tgmpa-install']) && 'install-plugin' === $_GET['tgmpa-install']) {
                ...
                if (true === $complete) {
                    echo '<p>', sprintf(esc_html($this->strings['complete']), '<a href="' . esc_url(self_admin_url()) . '">' . esc_html__('Return to the Dashboard', 'jobcareer') . '</a>'), '</p>';
            ...
            if (!empty($desired_slug)) {
                ...
                if (!empty($subdir_name) && $subdir_name !== $desired_slug) {
                    ...
                    if (true === $GLOBALS['wp_filesystem']->move($from, $to)) {
                        ...
                        return new WP_Error('rename_failed', esc_html__('The remote plugin package is does not contain a folder with the desired slug and renaming did not work.', 'jobcareer') . ' ' . esc_html__('Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'jobcareer'), array('found' => $subdir_name, 'expected' => $desired_slug));
                    }
                } elseif (empty($subdir_name)) {
                    return new WP_Error('packaged_wrong', esc_html__('The remote plugin package consists of more than one file, but the files are not packaged in a folder.', 'jobcareer') . ' ' . esc_html__('Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'jobcareer'), array('found' => $subdir_name, 'expected' => $desired_slug));
                }
            }
        ...
        protected function activate_single_plugin($file_path, $slug, $automatic = false) {
            ...
            if (!is_plugin_active($file_path)) {
                ...
                sprintf(
                        esc_html__('No action taken. Plugin %1$s was already active.', 'jobcareer'), '<strong>' . esc_html($this->plugins[$slug]['name']) . '</strong>'
                ),
                '</p></div>';
            }
        ...
        public function notices() {
            ...
            if (!empty($message)) {
                ...
                $line_template = '<span style="display: block; " class="cs-line-template">%s</span>' . "\n";
                ...
                foreach ($message as $type => $plugin_group) {
                    ...
                    $imploded = empty($linked_plugins) ? $last_plugin : ( implode(', ', $linked_plugins) . ' ' . esc_html_x('and', 'plugin A *and* plugin B', 'jobcareer') . ' ' . $last_plugin );

                    $rendered .= sprintf(
                            $line_template, sprintf(
                                    translate_nooped_plural($this->strings[$type], $count, 'jobcareer'), $imploded, $count
                            )
                    );
                ...
                $show_install_link = $install_link ? '<a href="' . esc_url($this->get_tgmpa_url()) . '">' . translate_nooped_plural($this->strings['install_link'], $install_link_count, 'jobcareer') . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url($this->get_tgmpa_url()) . '">' . translate_nooped_plural($this->strings['activate_link'], $activate_link_count, 'jobcareer') . '</a>' : '';
        ...
        protected function _get_plugin_basename_from_slug($slug) {
            ...
            if (!function_exists('get_plugins')) {
                jobcareer_include_file(ABSPATH . 'wp-admin/includes/plugin.php');
            }
        ...
        protected function get_wp_repo_download_url($slug) {
            ...
            if (!function_exists('plugins_api')) {
                jobcareer_include_file(ABSPATH . 'wp-admin/includes/plugin-install.php'); // Need for plugins_api.
            }
...
if (!class_exists('WP_List_Table')) {
    jobcareer_include_file(ABSPATH . 'wp-admin/includes/class-wp-list-table.php');
}
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ($this->tgmpa->plugins as $slug => $plugin) {
                ...
                if ('repo' !== $plugin['source'] && preg_match(TGM_Plugin_Activation::WP_REPO_REGEX, $plugin['source']) !== 1) {
                    if (preg_match(TGM_Plugin_Activation::IS_URL_REGEX, $plugin['source'])) {
                        ...
                        $table_data[$i]['source'] = esc_html__('External Source', 'jobcareer');
                    } else {
                        // The plugin is pre-packaged with the theme.
                        $table_data[$i]['source'] = esc_html__('Pre-Packaged', 'jobcareer');
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = esc_html__('WordPress Repository', 'jobcareer');
                }

                if (isset($plugin['required']) && $plugin['required']) {
                    $table_data[$i]['type'] = esc_html__('Required', 'jobcareer');
                } else {
                    $table_data[$i]['type'] = esc_html__('Recommended', 'jobcareer');
                }

                if (!isset($installed_plugins[$plugin['file_path']])) {
                    $table_data[$i]['status'] = sprintf('%1$s', esc_html__('Not Installed', 'jobcareer'));
                } elseif (is_plugin_inactive($plugin['file_path'])) {
                    $table_data[$i]['status'] = sprintf('%1$s', esc_html__('Installed But Not Activated', 'jobcareer'));
                }
        ...
        public function column_plugin($item) {
            ...
            if (!is_plugin_active($item['file_path']) && !isset($installed_plugins[$item['file_path']])) {
                ...
                $actions = array(
                    'install' => sprintf(
                            '<a href="%1$s">' . esc_html_x('Install %2$s', '%2$s = plugin name in screen reader markup', 'jobcareer') . '</a>', esc_url($install_nonce_url), '<span class="screen-reader-text">' . esc_html($item['sanitized_plugin']) . '</span>'
                    ),
                );
            }
            ...
            elseif (is_plugin_inactive($item['file_path'])) {
                ...
                $actions = array(
                    'activate' => sprintf(
                            '<a href="%1$s">' . esc_html_x('Activate %2$s', '%2$s = plugin name in screen reader markup', 'jobcareer') . '</a>', esc_url($activate_url), '<span class="screen-reader-text">' . esc_html($item['sanitized_plugin']) . '</span>'
                    ),
                );
            }
        ...
        public function no_items() {
            ...
            printf(wp_kses_post(esc_html__('No plugins to install or activate. <a href="%1$s">Return to the Dashboard</a>', 'jobcareer')), esc_url(self_admin_url()));
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        }
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => esc_html__('Plugin', 'jobcareer'),
                'source' => esc_html__('Source', 'jobcareer'),
                'type' => esc_html__('Type', 'jobcareer'),
                'status' => esc_html__('Status', 'jobcareer'),
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install' => esc_html__('Install', 'jobcareer'),
                'tgmpa-bulk-activate' => esc_html__('Activate', 'jobcareer'),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ('tgmpa-bulk-install' === $this->current_action()) {
                ...
                if (empty($plugins_to_install)) {
                    echo '<div id="message" class="error"><p>', esc_html__('No plugins are available to be installed at this time.', 'jobcareer'), '</p></div>';
                    return false;
                }
            ...
            if ('tgmpa-bulk-activate' === $this->current_action()) {
                ...
                if (empty($plugins_to_activate)) {
                    echo '<div id="message" class="error"><p>', esc_html__('No plugins are available to be activated at this time.', 'jobcareer'), '</p></div>';
                    return false;
                }
                ...
                if (is_wp_error($activate)) {
                    ...
                    $imploded = empty($plugin_names) ? $last_plugin : ( implode(', ', $plugin_names) . ' ' . esc_html_x('and', 'plugin A *and* plugin B', 'jobcareer') . ' ' . $last_plugin );

                    printf(
                            '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', esc_html(_n('The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'jobcareer')), $imploded // WPCS: xss ok
                    );
            ...
            if (!class_exists('TGM_Bulk_Installer')) {
                ...
                class TGM_Bulk_Installer extends WP_Upgrader {
                    ...
                    public function install_strings() {
                        ...
                        $this->strings['no_package'] = esc_html__('Install package not available.', 'jobcareer');
                        $this->strings['downloading_package'] = esc_html__('Downloading install package from <span class="code">%s</span>&#8230;', 'jobcareer');
                        $this->strings['unpack_package'] = esc_html__('Unpacking the package&#8230;', 'jobcareer');
                        $this->strings['installing_package'] = esc_html__('Installing the plugin&#8230;', 'jobcareer');
                        $this->strings['process_failed'] = esc_html__('Plugin install failed.', 'jobcareer');
                        $this->strings['process_success'] = esc_html__('Plugin installed successfully.', 'jobcareer');
                    }
                    ...
                    public function activate_strings() {
                        ...
                        $this->strings['activation_failed'] = esc_html__('Plugin activation failed.', 'jobcareer');
                        $this->strings['activation_success'] = esc_html__('Plugin activated successfully.', 'jobcareer');
                    }
            ...
            if (!class_exists('TGM_Bulk_Installer_Skin')) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        ...
                        $this->upgrader->strings['skin_update_failed_error'] = esc_html__('An error occurred while installing %1$s: <strong>%2$s</strong>.', 'jobcareer');
                        $this->upgrader->strings['skin_update_failed'] = esc_html__('The installation of %1$s failed.', 'jobcareer');

                        // Automatic activation strings.
                        if ($this->tgmpa->is_automatic) {
                            $this->upgrader->strings['skin_upgrade_start'] = esc_html__('The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'jobcareer');
                            $this->upgrader->strings['skin_update_successful'] = esc_html__('%1$s installed and activated successfully.', 'jobcareer') . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__('Show Details', 'jobcareer') . '</span><span class="hidden">' . esc_html__('Hide Details', 'jobcareer') . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end'] = esc_html__('All installations and activations have been completed.', 'jobcareer');
                            $this->upgrader->strings['skin_before_update_header'] = esc_html__('Installing and Activating Plugin %1$s (%2$d/%3$d)', 'jobcareer');
                        }
                        // Default installation strings.
                        else {
                            $this->upgrader->strings['skin_upgrade_start'] = esc_html__('The installation process is starting. This process may take a while on some hosts, so please be patient.', 'jobcareer');
                            $this->upgrader->strings['skin_update_successful'] = esc_html__('%1$s installed successfully.', 'jobcareer') . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__('Show Details', 'jobcareer') . '</span><span class="hidden">' . esc_html__('Hide Details', 'jobcareer') . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end'] = esc_html__('All installations have been completed.', 'jobcareer');
                            $this->upgrader->strings['skin_before_update_header'] = esc_html__('Installing Plugin %1$s (%2$d/%3$d)', 'jobcareer');
                        }
                    }
                    ...
                    public function before($title = '') {
                        ...
                        printf(
                                '<h4>' . wp_kses_post($this->upgrader->strings['skin_before_update_header']) . ' <img alt="" src="%4$s" class="hidden waiting-%5$s cs-updateheader-heading"  /></h4>', esc_html($this->plugin_names[$this->i]), absint($this->upgrader->update_current), absint($this->upgrader->update_count), esc_url(admin_url('images/wpspin_light.gif')), esc_attr($this->upgrader->update_current)
                        );
                    ...
                    public function after($title = '') {
                        ...
                        if (!empty($this->result) && !is_wp_error($this->result)) {
                            ...
                            sprintf(
                                    ...
                                    esc_html($this->plugin_names[$this->i]), 'jQuery(\'#progress-' . esc_js($this->upgrader->update_current) . '\').toggle();jQuery(\'span\', this).toggle(); return false;'
                            ),
                            '</p></div>',
                    ...
                    public function bulk_footer() {
                        ...
                        if (true === $complete) {
                            echo '<p>', sprintf(esc_html($this->tgmpa->strings['complete']), '<a href="' . esc_url(self_admin_url()) . '">' . esc_html__('Return to the Dashboard', 'jobcareer') . '</a>'), '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }
                    }
