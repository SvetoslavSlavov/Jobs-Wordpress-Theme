...
if (!class_exists('jobcareer_mega_menu_walker')) {
    ...
    class jobcareer_mega_menu_walker extends Walker_Nav_Menu {
        ...
        function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
            ...
            $attributes = isset($item->attr_title) ? ' title="' . esc_attr($item->attr_title) . '"' : '';
            $attributes .= isset($item->target) ? ' target="' . esc_attr($item->target) . '"' : '';
            $attributes .= isset($item->xfn) ? ' rel="' . esc_attr($item->xfn) . '"' : '';
            ...
            $item_output .= isset($args->description) ? ' <p>' . esc_attr($item->description) . '</p> ' : '';
...
if (!function_exists('jobcareer_breadcrumb_header')) {
    ...
    function jobcareer_breadcrumb_header($post_ID = '') {
        ...
        if ($cs_header_banner_style == 'breadcrumb_header') {
            $cs_sub_header_overlay_color = get_post_meta((int) $post_ID, "cs_subheader_overlay_color", true);
            $cs_overlay_opacity = get_post_meta((int) $post_ID, "cs_subheader_overlay_opacity", true);
        } else {
            $cs_sub_header_overlay_color = isset($jobcareer_options['cs_sub_header_overlay_color']) ? $jobcareer_options['cs_sub_header_overlay_color'] : '';
            $cs_overlay_opacity = isset($jobcareer_options['cs_overlay_opacity']) ? $jobcareer_options['cs_overlay_opacity'] : '0.5';
        }
        ...
        if (isset($header_banner_image) && $header_banner_image != '') {
            ...
            $px_upload_dir = wp_upload_dir();
            $px_upload_dir = isset($px_upload_dir['basedir']) ? $px_upload_dir['basedir'] . '/' : '';
            $px_images_dir = trailingslashit(get_template_directory()) . 'assets/images/';

            $px_img_name = preg_replace('/^.+[\\\\\\/]/', '', $header_banner_image);

            if (is_file($px_upload_dir . $px_img_name) || is_file($px_images_dir . $px_img_name)) {
        ...
        if (isset($page_subheader_parallax) && (string) $page_subheader_parallax == 'on') {
            echo '<script>jQuery(document).ready(function($){ "use strict"; cs_parallax_func()});</script>';
        ...
        ?>
        <div class="cs-subheader <?php  echo jobcareer_special_char($page_tile_align). ' '.  jobcareer_special_char($parallax_class); ?>" <?php echo jobcareer_special_char($subheader_style_elements); ?>> 
            <div class="container">
                <?php
                if (is_page()) {
